...
def registry = "ccontainerregistry.mts-lab.net/scorepal/" as String
def localRegistry = "http://192.168.1.227:8082/" as String
def localRegistryUrl = "http://192.168.1.227:8082" as String
def registryUrl = "containerregistry.mts-lab.net" as String
def registryBase = "containerregistry.mts-lab.net" as String
def appName = "scorepal-rest-svc" as String
                ...
                script{
                    dir("./"){
                        try{
                            ...
                            docker.withRegistry(localRegistryUrl,'scorepal-containerregistry') {

                                def smweb_l = docker.build("scorepal/${appName}:${s_branch}","./")

                                // or docker.build, etc.
                                def remoteImage = "${registryBase}/scorepal/${appName}:${s_branch}"
                                smweb_l.push()
                            ...
                            docker.withRegistry(registryUrl,'scorepal-containerregistry') {
                                // sh "docker system prune -a -f"

                                def smweb = docker.build("scorepal/${appName}:${s_branch}","./")
